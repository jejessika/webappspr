...
def homepage():
    if request.method == 'POST':
        ...
        ftype = fname.split('.')[1]
        storage.child(ftype+'/'+fname).put(ufile)
        flink = storage.child(ftype+'/'+fname).get_url(None)
        session['flink'] = flink
...
def content():    
    if 'flink' in session:
        ...
        TOOLTIPS = [
            ("Index", "$index"),
            ("Time (s)", "$x"),
            ("Reflectivity Unit (RU)", "$y"),
        ]

        #Add plot
        p= figure(title="Data SPR Hasil Algoritma", x_axis_label="Time (s)", y_axis_label="Reflectivity Unit (RU)", tooltips = TOOLTIPS, toolbar_location = "above", tools = "pan, hover, box_zoom, reset, wheel_zoom")
            
        #Render glyph
        #Put the legend outside of the plot
        legend = Legend(items=[
            ("Channel 1", [p.line(x,y, line_width=2, muted_alpha = 0.2)]),("Baseline",[p.line(data_x, base_y, color='red', line_width=2, muted_alpha=0.2)])
        ], location="center")
        ...
        return render_template('content.html', **kwargs)
